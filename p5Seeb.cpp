...
int process(vector <string> &v, vector <long long int> &seeb, vector <int> &processed, int pos){
    ...
    vector <pair<long long int, long long int>> newRanges;
    ...
    for(int i = pos + 1; i < n; i++){
        ...
        for(int j = 0; j < m; j++){
            if(seeb[j] >= rs && seeb[j] < re){
                if(!processed[j]) seeb[j] = temp[0] + (seeb[j] - rs);
                processed[j] = 1;
            }
        }
    }
...
int main(){
    ...
    vector <pair<long long int, long long int>> seeb = getSeeds(inputlines[0]);
